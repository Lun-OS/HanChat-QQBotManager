...
func RegisterSettingsRoutes(r *gin.RouterGroup, base *zap.Logger, accountConfig *config.AccountConfig, reverseWS *services.ReverseWebSocketService) {
	...
	r.POST("/appearance", func(c *gin.Context) {
		...
		// 记录设置修改日志
		if lm := getLogManager(reverseWS); lm != nil {
			lm.WriteSettingLog("appearance", "update", c.ClientIP(),
				fmt.Sprintf("theme=%s fontSize=%d", body.Theme, body.FontSize))
		}
	...
	r.GET("/logs", func(c *gin.Context) {
		...
		// 实际读取设置修改日志
		logManager := getLogManager(reverseWS)
		if logManager == nil {
		...
		// 读取 "system" 账号的 setting 类型日志
		// limit 使用 pageSize，与前端分页参数保持一致
		logs, err := logManager.GetLogs("system", "setting", "", pageSize*page)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"message": "读取设置日志失败: " + err.Error(),
			})
			return
		}
		...
		for _, l := range logs {
			if level == "" || strings.Contains(l, "["+level+"]") {
	...
	r.GET("/admins", func(c *gin.Context) {
		...
		// 读取管理员日志作为管理员列表的辅助数据
		logs, err := logManager.GetLogs("system", "admin", "", 50)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"message": "读取管理员日志失败: " + err.Error(),
			})
			return
		}
	...
	r.GET("/operations", func(c *gin.Context) {
		...
		logs, err := logManager.GetLogs("system", "operation", "", pageSize*page)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"message": "读取操作日志失败: " + err.Error(),
			})
			return
		}
		...
		filtered := make([]string, 0, len(logs))
		for _, l := range logs {
			if category == "" || strings.Contains(l, "[category:"+category+"]") {
				filtered = append(filtered, l)
			}
		}
...
// getLogManager 安全地获取日志管理器
func getLogManager(reverseWS *services.ReverseWebSocketService) *services.LogManager {
	if reverseWS == nil {
		return nil
	}
	return reverseWS.GetLogManager()
}
