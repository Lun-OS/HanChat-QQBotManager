...
// respondPluginError 统一的插件错误响应
// 自动识别 PluginError 并附带 err_code 给前端
func respondPluginError(c *gin.Context, err error) {
	if err == nil {
		return
	}
	var pe *plugins.PluginError
	if errors.As(err, &pe) {
...
func RegisterPluginManageRoutes(r *gin.RouterGroup, pm *plugins.Manager, base *zap.Logger) {
	...
	r.POST("/reload", func(c *gin.Context) {
		...
		logger.Infow("重启插件", "self_id", body.SelfID, "name", body.Name)
		if err := pm.Reload(body.SelfID, body.Name); err != nil {
			respondPluginError(c, err)
			return
		}

		c.JSON(http.StatusOK, gin.H{"success": true, "message": "插件已重启"})
	})
