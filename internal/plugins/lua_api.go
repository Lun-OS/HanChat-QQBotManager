...
// 系统/获取Cookies
// 参数：domain (string)，可选参数 target_self_id (string) - 指定目标机器人账号，不传则使用当前插件归属的机器人
// 返回：table（与 LLOneBot /get_cookies 返回结构一致），错误时返回 nil, error
func (m *Manager) luaSystemCookies(instance *LuaPluginInstance) func(*lua.LState) int {
	return func(L *lua.LState) int {
		// 检查服务
		if err := checkLLService(L, instance); err != nil {
		...
		// 可选参数：指定目标机器人账号
		targetSelfID := L.OptString(2, instance.SelfID)
		if targetSelfID == "" {
			return luaAPIError(L, fmt.Errorf("无法确定目标机器人账号"), "获取Cookies失败")
		}

		// 调用 LLOneBot 获取 cookies
		var result map[string]interface{}
		if instance.reverseWS != nil {
			result, err = instance.reverseWS.CallBotAPI(targetSelfID, "/get_cookies", map[string]interface{}{
				"domain": domain,
			})
		} else if instance.llService != nil {
			result, err = instance.llService.CallAPI("/get_cookies", map[string]interface{}{
				"domain": domain,
			}, "POST")
		} else {
		...
		if err != nil {
			return luaAPIError(L, err, fmt.Sprintf("获取Cookies失败 [domain=%s, target=%s]", domain, targetSelfID))
		}
...
// 系统/获取内存信息
// 返回一个表，包含每个运行中插件的内存占用详情
// {
//   total = number,     -- 所有插件总内存（字节）
//   plugins = {
//     { name = string, memory = number, memory_mb = string },
//     ...
//   }
// }
func (m *Manager) luaSystemGetMemory(selfID string) func(*lua.LState) int {
	return func(L *lua.LState) int {
		container := m.getContainer(selfID)
		if container == nil {
			result := L.NewTable()
			L.SetField(result, "total", lua.LNumber(0))
			L.SetField(result, "plugins", L.NewTable())
			L.Push(result)
			return 1
		}

		container.mu.RLock()
		plugins := make([]*LuaPluginInstance, 0, len(container.LuaPlugins))
		for _, inst := range container.LuaPlugins {
		...
		for i, inst := range plugins {
			mem := atomic.LoadInt64(&inst.memoryUsage)
			if mem == 0 {
				// mem = estimateLuaMemory(inst.L)
			}
			totalMem += mem

			pluginTbl := L.NewTable()
			L.SetField(pluginTbl, "name", lua.LString(inst.Name))
			L.SetField(pluginTbl, "memory", lua.LNumber(mem))
			L.SetField(pluginTbl, "memory_mb", lua.LString(fmt.Sprintf("%.2f", float64(mem)/1024/1024)))

			L.RawSetInt(pluginsTbl, i+1, pluginTbl)
		}

		result := L.NewTable()
		L.SetField(result, "total", lua.LNumber(totalMem))
		L.SetField(result, "total_mb", lua.LString(fmt.Sprintf("%.2f", float64(totalMem)/1024/1024)))
		L.SetField(result, "plugins", pluginsTbl)
