...
# Start program with debug mode
# Note: The Go program will look for 'web' folder in current working directory
try {
    Write-Host "Starting in DEBUG mode (--debug)" -ForegroundColor Magenta
    $process = Start-Process -FilePath $ExePath -ArgumentList "--debug" -PassThru -Wait -NoNewWindow
