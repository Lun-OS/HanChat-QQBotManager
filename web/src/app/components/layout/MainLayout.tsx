...
function StarFieldBackground({ light = false }: { light?: boolean }) {
  // 性能模式下减少粒子数量，避免移动端卡顿
  const stars = useMemo(() => [...Array(light ? 18 : 50)].map((_, i) => {
...
export function MainLayout() {
  ...
  return (
    <div className='h-screen relative flex items-stretch overflow-hidden bg-gray-50 dark:bg-[#050810]'>
      <StarFieldBackground light={performanceMode} />
      ...
      <motion.div
        layout
        ref={contentRef}
        initial={performanceMode ? false : { opacity: 0, scale: 0.98 }}
        animate={performanceMode ? false : { opacity: 1, scale: 1 }}
        transition={performanceMode ? { duration: 0 } : { duration: 0.4 }}
        ...
        <div className='h-10 flex items-center font-bold text-xl backdrop-blur-lg rounded-full bg-white/70 dark:bg-black/30 shadow-sm dark:shadow-none border border-gray-200/50 dark:border-white/[0.06] m-2 mb-0 flex-shrink-0'>
          ...
          <div className='flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-400'>
            {title.map((item, index) =>
              performanceMode ? (
                <div key={`${item}-${index}`} className='text-gray-900 dark:text-white font-medium'>
                  {item}
                </div>
              ) : (
