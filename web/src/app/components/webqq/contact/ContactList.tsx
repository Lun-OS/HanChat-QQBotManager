...
import { filterGroups, formatMessageTime } from '../../../services/webqqApi'
...
function FriendCategoryList({ categories, selectedPeerId, onSelect }: FriendCategoryListProps) {
...
function GroupListView({ items, unreadCounts, selectedPeerId, onSelect, groupLastTimeMap, subtitleMap, showPinnedStyle = false }: GroupListViewProps) {
...
function RecentList({ items, unreadCounts, selectedPeerId, onSelect, onEnterGroupAssistant }: RecentListProps) {
  ...
  const visibleRecentItems = useMemo(() => {
    return items.filter(item => {
      if (item.chatType !== 2) return true
  ...
  const getDisplayName = (item: RecentChatItem): string => {
    if (item.chatType === 2) {
      const group = groups.find(g => g.groupCode === item.peerId)
      if (group?.remarkName && group.remarkName !== group.groupName) return group.remarkName
      ...
      for (const category of friendCategories) {
        ...
        if (friend?.remark && friend.remark !== friend.nickname) return friend.remark
      }
    }
...
export function RecentListItem({ item, displayName, unreadCount, isSelected, onClick, onContextMenu }: RecentListItemProps) {
  return (
    <div
      onClick={onClick}
...
function GroupAssistantList({ groups, recentChats, groupLastTimeMap, groupLastMessageMap, unreadCounts, selectedPeerId, onSelect, onBack }: GroupAssistantListProps) {
  const assistantGroups = useMemo(() => groups.filter(g => g.msgMask === 2), [groups])
