...
// 快速导入解析结果
interface QuickImportParseResult {
  pluginName: string;
  mainLua: string;
  extraFiles: Array<{ name: string; content: string }>;
}

// 快速导入解析：解析头部注释中的 @plugin/@file/@end 标记
const parseQuickImport = (code: string): QuickImportParseResult => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const commentContent = commentMatch ? commentMatch[1].trim() : null;

    // @plugin: <name> - 标记插件名
    const pluginMatch = commentContent ? commentContent.match(/^@plugin\s*[:：]\s*(.+)$/) : null;
    if (pluginMatch) {
      headerStarted = true;
      pluginName = pluginMatch[1].trim();
      continue;
    }

    // @file <name> - 开始一个新文件
    const fileMatch = commentContent ? commentContent.match(/^@file\s+(.+)$/) : null;
    if (fileMatch) {
      headerStarted = true;
      // 提交上一个未关闭的文件
      if (currentFile) {
      ...
      currentFile = { name: fileMatch[1].trim(), lines: [] };
      continue;
    }

    // @end - 头部结束
    if (commentContent && /^@end\b/.test(commentContent)) {
      headerStarted = true;
      if (currentFile) {
        extraFiles.push({
          name: currentFile.name,
    ...
    // 在 @file 块内的注释行：作为文件内容
    if (currentFile && commentContent !== null) {
      headerStarted = true;
      currentFile.lines.push(commentContent);
      continue;
    }

    // 非 @ 指令的注释行（在头部但不在 @file 块内）：忽略
    if (commentContent !== null) {
      if (!headerStarted) continue;  // 文件开头就只是注释，跳过
      continue; // 头部内的普通注释，忽略
    }

    // 空行
    if (trimmed === '') {
      if (headerStarted || !headerStarted) continue;
    }

    // 非空非注释行：要么是 main.lua 的开头（无头部），要么是头部的结束
    mainStartIdx = i;
    break;
  }
  ...
  // 截取 main.lua 内容
  const mainRaw = lines.slice(mainStartIdx).join('\n');
  const mainLua = mainRaw.replace(/^\s*\n/, '');

  return { pluginName, mainLua, extraFiles };
};
...
const QUICK_IMPORT_TEMPLATE = `-- @plugin: my-plugin
-- @file config.json
-- {
--   "enabled": true,
--   "version": "1.0.0"
...
const QuickImportDialog: React.FC<QuickImportDialogProps> = ({ isOpen, onConfirm, onCancel }) => {
  const [code, setCode] = useState(QUICK_IMPORT_TEMPLATE);
  const [preview, setPreview] = useState<QuickImportParseResult | null>(null);

  useEffect(() => {
    if (isOpen) {
      setCode(QUICK_IMPORT_TEMPLATE);
      setPreview(parseQuickImport(QUICK_IMPORT_TEMPLATE));
    }
  }, [isOpen]);
  ...
  const handleCodeChange = (value: string) => {
    setCode(value);
    setPreview(parseQuickImport(value));
  };

  const handleSubmit = () => {
    if (!code.trim()) {
      toast.error('请输入 Lua 代码');
      return;
    }
    const result = parseQuickImport(code);
    if (!result.pluginName) {
      toast.error('未检测到插件名称，请使用 -- @plugin: 名称 标注');
      return;
    }
    onConfirm(code);
  };
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.9, opacity: 0 }}
        className="bg-white dark:bg-[#1D2129] dark:backdrop-blur-xl border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl w-full max-w-5xl h-[85vh] flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-full bg-blue-100 text-blue-600 dark:bg-white/10 dark:text-white">
              <Zap className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">快速导入插件</h3>
              <p className="text-xs text-gray-500">在代码头部使用注释标注插件信息即可一键创建</p>
            </div>
          </div>
          <button
            onClick={onCancel}
        ...
        <div className="flex-1 grid grid-cols-2 gap-0 min-h-0 overflow-hidden">
          {/* 左侧：代码编辑 */}
          <div className="flex flex-col border-r border-gray-200 dark:border-gray-700 min-h-0">
            <div className="flex items-center gap-2 p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.03]">
              <FileCode className="w-4 h-4 text-blue-500" />
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Lua 代码</span>
              <span className="ml-auto text-xs text-gray-500">{code.length} 字符</span>
            </div>
          ...
          <div className="flex flex-col min-h-0">
            <div className="flex items-center gap-2 p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.03]">
              ...
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">解析预览</span>
            </div>
            <div className="flex-1 overflow-auto p-4 space-y-4">
              {/* 插件名 */}
              <div className="p-3 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-gray-700">
                <div className="text-xs text-gray-500 mb-1">插件名 (来自 -- @plugin:)</div>
                <div className="text-sm font-mono text-gray-900 dark:text-white">
                  {preview?.pluginName || <span className="text-red-500">未设置（必填）</span>}
                </div>
              </div>

              {/* 额外文件 */}
              <div className="p-3 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-gray-700">
                <div className="text-xs text-gray-500 mb-2">附加文件 (来自 -- @file)</div>
                {preview?.extraFiles && preview.extraFiles.length > 0 ? (
                  <ul className="space-y-2">
                    {preview.extraFiles.map((f, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm">
                        <File className="w-4 h-4 mt-0.5 text-gray-400" />
                        <div className="flex-1 min-w-0">
                          <div className="font-mono text-gray-900 dark:text-white">{f.name}</div>
                          <div className="text-xs text-gray-500">{f.content.length} 字符</div>
                        </div>
                      </li>
                    ))}
                  </ul>
              ...
              {/* main.lua 内容预览 */}
              <div className="p-3 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-gray-700">
                <div className="text-xs text-gray-500 mb-2">main.lua ({preview?.mainLua.length || 0} 字符)</div>
                <pre className="text-xs font-mono text-gray-800 dark:text-gray-300 whitespace-pre-wrap break-all max-h-60 overflow-auto">
                  {preview?.mainLua || '(空)'}
                </pre>
              </div>
              ...
              <details className="text-xs text-gray-500">
                <summary className="cursor-pointer hover:text-gray-700 dark:hover:text-gray-300">格式说明</summary>
                <div className="mt-2 p-3 rounded bg-gray-50 dark:bg-white/[0.02] font-mono space-y-1">
                  <div>-- @plugin: 插件名（必填）</div>
                  <div>-- @file 文件名</div>
                  <div>-- 文件内容（每行都以 -- 开头）</div>
                  <div>-- @end （标记头部结束）</div>
                  <div className="pt-1">-- 头部之后的内容作为 main.lua</div>
                </div>
              </details>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between p-4 border-t border-gray-200 dark:border-gray-700">
          <div className="text-xs text-gray-500">
            提示：附加文件（如 config.json）会连同 main.lua 一同创建
          </div>
...
export function PluginManager() {
  ...
  const [showQuickImportDialog, setShowQuickImportDialog] = useState(false);
  const [quickImportSide, setQuickImportSide] = useState<'left' | 'right'>('right');
  ...
  // 快速导入：根据 Lua 代码头部注释一键创建插件
  const handleQuickImport = async (code: string) => {
    const result = parseQuickImport(code);
    if (!result.pluginName) {
      toast.error('未检测到插件名称，请使用 -- @plugin: 名称 标注');
      return;
    ...
    const targetSelfId = quickImportSide === 'left' ? leftSelfId : rightSelfId;
    const parentPath = `/plugins/${targetSelfId}`;
    const pluginPath = `${parentPath}/${result.pluginName}`;

    try {
      // 1. 创建插件文件夹
      const folderRes = await pluginManagerApi.createFile(parentPath, result.pluginName, 'folder');
      if (!folderRes.success) {
        toast.error(folderRes.message || '创建插件文件夹失败');
        return;
      }
      ...
      // 2. 创建 main.lua
      const mainRes = await pluginManagerApi.createFile(pluginPath, 'main.lua', 'file');
      if (!mainRes.success) {
        toast.error(mainRes.message || '创建 main.lua 失败');
        return;
      }
      if (result.mainLua) {
        const writeRes = await pluginManagerApi.writeFile(`${pluginPath}/main.lua`, result.mainLua);
        if (!writeRes.success) {
          toast.error(writeRes.message || '写入 main.lua 失败');
          return;
        }
      }
      ...
      for (const f of result.extraFiles) {
        const createRes = await pluginManagerApi.createFile(pluginPath, f.name, 'file');
        if (!createRes.success) {
          toast.warning(`${f.name} 创建失败：${createRes.message || '未知错误'}`);
          continue;
        }
        if (f.content) {
          const writeRes = await pluginManagerApi.writeFile(`${pluginPath}/${f.name}`, f.content);
          if (!writeRes.success) {
            toast.warning(`${f.name} 写入失败：${writeRes.message || '未知错误'}`);
          }
        }
      }

      toast.success(`插件 "${result.pluginName}" 导入成功`);
      if (quickImportSide === 'left') {
        loadLeftFiles(leftSelfId);
      } else {
        loadRightFiles(rightSelfId);
      }
    ...
    setShowQuickImportDialog(false);
  };
    ...
    return (
      <div className="flex items-center gap-2 p-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#2A2E38]">
        ...
        {canWrite && (
          <button
            onClick={() => {
              setQuickImportSide(side);
              setShowQuickImportDialog(true);
            }}
            className="p-1.5 text-gray-600 dark:text-gray-400 hover:text-[#165DFF] dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/[0.05] rounded transition-colors"
            title="快速导入（通过 Lua 代码头部注释一键创建插件）"
          >
            <Zap className="w-4 h-4" />
          </button>
        )}
    ...
    <div className="flex flex-col h-full">
      ...
      <AnimatePresence>
        ...
        {showQuickImportDialog && (
          <QuickImportDialog
            isOpen={showQuickImportDialog}
            onConfirm={handleQuickImport}
            onCancel={() => setShowQuickImportDialog(false)}
          />
        )}
